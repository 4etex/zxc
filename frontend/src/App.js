  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
      ...
      <div className="flex gap-4 mb-6">
        ...
        >
          {generating ? "⏳ Генерируем..." : generateVideos ? "🎬 Создать контент + видео" : "🎯 Создать контент"}
        </button>
        
        {content && content.content && content.content.telegram && (
          <button
      ...
      {content && content.content && (
        <div className="space-y-4">
          ...
          {Object.entries(content.content).map(([platform, items]) => (
                    ...
                    {item.affiliate_links && (
                      <div className="mt-2">
                        <span className="text-xs text-green-600">
                          💰 Добавлено {item.affiliate_links.length} партнерских ссылок
                        </span>
                      </div>
                    )}
                  </div>
      ...
      {content && content.videos && (
        <div className="mt-6 space-y-4">
          <h3 className="font-semibold text-lg">🎬 Созданные видео:</h3>
          {Object.entries(content.videos).map(([platform, videos]) => (
            <div key={platform} className="border rounded-lg p-4 bg-gradient-to-r from-purple-50 to-pink-50">
              <h4 className="font-medium mb-3 capitalize">
                {platform.replace('_', ' ')} ({videos.length} видео)
              </h4>
                ...
                {videos.map((video, idx) => (
                  <div key={idx} className="bg-white p-3 rounded border">
                    ...
                    <div className="text-xs text-gray-600 mb-2">
                      Длительность: {video.duration}с | Платформа: {video.platform}
                    </div>
                    <div className="text-xs text-green-600">
                      ✅ Видео создано с озвучкой
                    </div>
                  </div>
                ))}
              </div>
